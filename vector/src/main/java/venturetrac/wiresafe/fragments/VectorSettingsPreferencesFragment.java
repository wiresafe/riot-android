...
package venturetrac.wiresafe.fragments;
...
import venturetrac.wiresafe.activity.CommonActivityUtils;
...
import venturetrac.wiresafe.contacts.ContactsManager;
import venturetrac.wiresafe.ga.GAHelper;
import venturetrac.wiresafe.gcm.GcmRegistrationManager;
import venturetrac.wiresafe.preference.ProgressBarPreference;
import venturetrac.wiresafe.preference.UserAvatarPreference;
import venturetrac.wiresafe.preference.VectorCustomActionEditTextPreference;
import venturetrac.wiresafe.util.PhoneNumberUtils;
import venturetrac.wiresafe.util.PreferencesManager;
import venturetrac.wiresafe.util.ResourceUtils;
import venturetrac.wiresafe.util.SharedDataItem;
import venturetrac.wiresafe.util.ThemeUtils;
import venturetrac.wiresafe.util.VectorUtils;
...
public class VectorSettingsPreferencesFragment extends PreferenceFragment implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    //Firebase user - for change password
...
//                        if (true) return;
                        ...
                        mSession.updatePassword(oldPwd, newPwd, new ApiCallback<Void>() {
                            ...
                            @Override
                            public void onSuccess(Void info) {
                                AuthCredential credential = EmailAuthProvider.getCredential(mFirebaseUser.getEmail(), oldPwd);
                                        ...
                                        if (task.isSuccessful()) {
                                            mFirebaseUser.updatePassword(newPwd).addOnCompleteListener(new OnCompleteListener<Void>() {
                                                ...
                                                public void onComplete(@NonNull Task<Void> task) {
                                                    ...
                                                    reAuthenticate(mFirebaseUser.getEmail(), newPwd);
                                                }
                                            });
                                        }
                                    }
    ...
    private void reAuthenticate(String email, String newPwd) {
